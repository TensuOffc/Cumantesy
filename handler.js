...
module.exports = {
      ...
      try {
        ...
        try {
          ...
          if (user) {
            ...
            if (!isNumber(user.tuna)) user.tuna = 0
        ...
        for (let name in global.plugins) {
          ...
          if ((usedPrefix = (match[0] || '')[0])) {
            ...
            if (!isPrems && plugin.limit && db.data.users[m.sender].limit < plugin.limit * 1) {
              this.reply(m.chat, `⚠ Limit anda habis, silahkan beli melalui *${usedPrefix}buylimit (jumlah)*\nAtau Ketik :\n.hourly\n.daily\n.weekly\n.monthly\n.yearly\nUntuk mendapatkan free limit!`, m)
              continue // Limit habis
            }
            if (plugin.level > _user.level) {
              this.reply(m.chat, `Butuh Level ${plugin.level} Untuk Menggunakan Fitur Ini, Level Mu ${_user.level}`, m)
          ...
          if (chat.welcome) {
            ...
            for (let user of participants) {
              ...
              try {
                ...
                text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Selamat Datang, @user!').replace('@subject', await this.getName(id)).replace('@desc', groupMetadata.desc) : (chat.sBye || this.bye || conn.bye || 'Bye Bye, @user!')).replace('@user', '@' + user.split('@')[0])
    ...
    async delete(message) {
      try {
        ...
        await this.reply(msg.key.remoteJid, `Terdeteksi @${participant.split`@`[0]} Menghapus Pesan\n\nUntuk Mematikan fitur ini ketik *.enable delete*`.trim(), msg, {
  ...
  conn.ws.on('CB:call', async function callUpdatePushToDb(json) {
    ...
    let sentMsg = await conn.reply(callerId, `[❗] SISTEM BLOCK OTOMATIS, JANGAN MENELEPON BOT!, Silahkan Chat Owner Bot, Untuk Membuka Chat Kembali!`)
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: '_*「 Fitur Khusus Owner Bot 」*_',
    owner: '_*「 Fitur Khusus Owner Bot 」*_',
    mods: '_*「 Fitur Khusus Mods Bot 」*_',
    premium: '_Fitur Khusus *Premium User*! Ketik .buyprem/.infosewa/.premium untuk cek list *Premium*_',
    banned: '_*「 Hanya Khusus Untuk User Terbanned 」*_',
    group: '_*「 Fitur Khusus Di Dalam Group 」*_',
    private: '_*「 Fitur Khusus Di Dalam Private Chat 」*_',
    admin: '_*「 Fitur Khusus Admin Group 」*_',
    botAdmin: '*_Jadikan bot sebagai *Admin* untuk menggunakan perintah ini!_*',
    unreg: '_Hi user, Untuk melanjutkan menggunakan bot, tolong daftar terlebih dahulu! Ketik Seperti:_\n\n◦ Contoh : *.daftar Namamu.Umurmu*\n◦ Contoh Penulisan : *.daftar Tensu.20*\n_Jangan Lupa Pakai . Sesuai contoh, Nama dan Umur bebas!_',
    restrict: '_Fitur ini di *disable*!_'
  } [type]
