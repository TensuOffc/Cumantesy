async function handler(m, { command }) {
	...
	switch (command) {
		...
		case 'leave': {
			...
			m.reply('✅ Anda Sudah Memberhentikan Chat')
		...
		case 'start': {
			if (Object.values(this.anonymous).find(room => room.check(m.sender))) return this.reply(m.chat, '⚠ _Kamu masih berada di dalam anonymous chat, menunggu partner membalas chat_\n\nketik *.leave* untuk keluar', m)
			let room = Object.values(this.anonymous).find(room => room.state === 'WAITING' && !room.check(m.sender))
			if (room) {
				await this.reply(room.a, '✅ _Partner ditemukan!_\n Ketik .sendkontak untuk saling mengirim kontak masing masing!', m)
				room.b = m.sender
				room.state = 'CHATTING'
