let handler = async (m, { conn, text, usedPrefix }) => {
  ...
  if (!text && !m.quoted)
    return conn.reply(m.chat, `⚠️ Berikan nomor, tag atau reply chat target`, m);
  // let exists = await conn.isOnWhatsApp(number)
  // if (exists) return conn.reply(m.chat, `*Nomor target tidak terdaftar di WhatsApp*`, m)
  if (isNaN(number)) return conn.reply(m.chat, `⚠️ Nomor tidak valid!`, m);
  if (number.length > 15) return conn.reply(m.chat, `⚠️ Format tidak valid!`, m);
  try {
    if (text) {
    ...
    if (!user)
      return conn.reply(
        m.chat,
        `⚠️ Target atau Nomor tidak ditemukan, mungkin sudah keluar atau bukan anggota grup ini`,
        m
      );
    if (user === m.sender)
      return conn.reply(m.chat, `⚠️ Tidak bisa berpacaran dengan diri sendiri`, m);
    //if (user === conn.user.jid)
    //return conn.reply(m.chat, `Tidak bisa berpacaran dengan bot`, m);

    if (typeof global.db.data.users[user] == "undefined")
      return m.reply("⚠️ Tidak terdaftar di database");
    ...
    if (
      ...
      if (global.db.data.users[pacar].pasangan == user) {
        var denda = Math.ceil((global.db.data.users[m.sender].money  / 100000) * 20);
        global.db.data.users[m.sender].money  -= denda;
        if (
          m.sender == pacar &&
          global.db.data.users[m.sender].pasangan == user
        ...
        conn.reply(
          m.chat,
          `⚠️ Tau sopan santun dikit teman\n@${
            user.split("@")[0]
