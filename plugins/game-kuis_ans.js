...
handler.before = async function (m, { text }) {
  ...
  if (!(id in conn.kuis)) return m.reply('⚠️ Soal itu telah berakhir, Ketik .kuis Untuk memulai sesi baru!')
  if (m.quoted.id == conn.kuis[id][0].id) {
  let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.kuis[id][3])
            delete this.kuis[id]
