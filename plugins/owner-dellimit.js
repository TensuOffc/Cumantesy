let handler = async (m, { conn, text }) => {
  if (!text) {
    throw '⚠️ Masukkan jumlah limit yang ingin dikurangi pada pengguna. Contoh: .dellimit @user 10';
  }

  let mentionedJid = m.mentionedJid[0];
  if (!mentionedJid) {
    throw '⚠️ Tag pengguna yang ingin dikurangi limitnya. Contoh: .dellimit @user 10';
  }

  let pointsToSubtract = parseInt(text.split(' ')[1]);
  if (isNaN(pointsToSubtract)) {
    throw '⚠️ Jumlah limit yang dimasukkan harus berupa angka. Contoh: .dellimit @user 10';
  }
