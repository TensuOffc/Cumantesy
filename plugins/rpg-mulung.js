...
let handler = async (m, {
  ...
  if (new Date - global.db.data.users[m.sender].lastmulung < 10800000) throw `⚠️ Anda sudah lelah untuk mulung\nTunggu selama ${msToTime(time - new Date())} lagi`
  let botolnye = `${Math.floor(Math.random() * 300)}`.trim()
  let kalengnye = `${Math.floor(Math.random() * 300)}`.trim()
  ...
  setTimeout(() => {
    conn.reply(m.chat, `Yuk waktunya mulung lagi cuy 🗑`, m)
  }, timeout)
}
handler.help = ['mulung']
handler.tags = ['rpg']
