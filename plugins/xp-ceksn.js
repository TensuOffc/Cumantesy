...
let handler = async function (m, { conn, text, usedPrefix }) {
  ...
  if (new Date() - db.data.users[m.sender].snlast > +86400000) {
    ...
    m.reply(`🏷 Your Serial Number: ${sn}

◦Jika Ingin Unreg, Salin Dan Ketik Ini: 
${usedPrefix}unreg ${sn}`.trim()
    )
  } else
    m.reply(
      `✅ Unregistered successfully, wait ${waktutionskh} to return unregistered`
    )
}
